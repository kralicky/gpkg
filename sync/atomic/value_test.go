...
func TestValueConcurrent(t *testing.T) {
	tests := [][]uint64{
		{uint64(0), ^uint64(0), uint64(1 + 2<<8), uint64(3 + 4<<8)},
		{uint64(0), ^uint64(0), uint64(1 + 2<<16), uint64(3 + 4<<16)},
...
var Value_SwapTests = []struct {
	...
	{init: 0, new: 0, want: 0, err: nil},
	{init: 0, new: 5, want: 0, err: nil},
	{init: 5, new: 10, want: 5, err: nil},
	{init: 10, new: 0, want: 10, err: nil},
}
...
var Value_CompareAndSwapTests = []struct {
	...
	{init: false, new: true, old: true, want: false, err: nil},
	{init: false, new: true, old: false, want: true, err: nil},
